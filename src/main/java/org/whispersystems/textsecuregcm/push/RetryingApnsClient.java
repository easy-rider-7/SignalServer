...
public class RetryingApnsClient {
  ...
  private static X509Certificate initializeCertificate(String pemCertificate) throws IOException {
    java.io.FileInputStream fileStream = new java.io.FileInputStream(pemCertificate);
    int avail = fileStream.available();
    PEMReader reader = new PEMReader(new InputStreamReader(fileStream));
    X509Certificate cer = (X509Certificate) reader.readObject();
    avail = fileStream.available();
    return cer;
  }

  private static PrivateKey initializePrivateKey(String pemKey) throws IOException {
    java.io.FileInputStream fileStream = new java.io.FileInputStream(pemKey);
    int avail = fileStream.available();
    PEMReader reader = new PEMReader(new InputStreamReader(fileStream));
    try 
    {   
